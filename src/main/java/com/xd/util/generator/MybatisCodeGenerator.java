...
public class MybatisCodeGenerator {
    ...
    private static final String schemaName = "ry_20220712";   // 数据库名称，必填
    private static final String[] table = {"member", "Member"};   // 必填，第一个是表名，第二个是实体类的名字
    private static final String modelName = "视频";   // 必填

    //=========================================以上内容必填===================================================//

    static {
        // 必填
        ds.setUrl("jdbc:mysql://localhost:3306/" + schemaName + "?serverTimezone=UTC&useUnicode=true&characterEncoding=utf-8&useSSL=false&allowPublicKeyRetrieval=true");
        ds.setUsername("root");
        ds.setPassword("root");
    }

    private static final String BaseFilePath = System.getProperty("user.dir") + "/src/main/java/com/xd/";
    ...
    static void createMapper(String entityName) {
        ...
        String format = StrUtil.format(FileUtil.readUtf8String(BaseFilePath + "/util/generator/template/mapper.template"), map);
    ...
    static void createService(String entityName) {
        ...
        String format = StrUtil.format(FileUtil.readUtf8String(BaseFilePath + "/util/generator/template/service.template"), map);
    ...
    static void createController(String entityName) {
        ...
        String format = StrUtil.format(FileUtil.readUtf8String(BaseFilePath + "/util/generator/template/controller.template"), map);
    ...
    static void createXml(String entityName) {
        Map<String, Object> map = new HashMap<>();
        map.put("entityName", entityName);
        String format = StrUtil.format(FileUtil.readUtf8String(BaseFilePath + "/util/generator/template/mapper_xml.template"), map);
    ...
    static void createVueHtml(String entityName, String tableName) throws SQLException {
        ...
        String format = StrUtil.format(FileUtil.readUtf8String(BaseFilePath + "/util/generator/template/vue.template"), map);
